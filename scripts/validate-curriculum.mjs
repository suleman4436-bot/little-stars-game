...
const countSource = fs.readFileSync(path.resolve('src/countStarsGame.tsx'), 'utf8');
...
assert.match(source, /englishCountTasks = Array\.from\(\{ length: 10 \}/); assert.match(source, /urduCountTasks = Array\.from\(\{ length: 10 \}/); assert.match(source, /visualCount: number/); assert.match(source, /wordUr: language === 'ur' \? urduNumbers\[value\]/);
...
assert.doesNotMatch(countSource, /Letter/); assert.match(countSource, /count-star-display/); assert.match(countSource, /options\.map/); assert.match(countSource, /disabled=\{result !== null \|\| transitioning\}/);
assert.match(appSource, /const countAnswer/); assert.match(appSource, /1200/); assert.match(appSource, /clearCountTimer/); assert.match(appSource, /selectedActivity\.id === 'count-stars'/);
