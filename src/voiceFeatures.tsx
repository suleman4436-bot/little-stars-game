...
export function ParentVoiceSettings({ profile, onUpdate }: { profile: Profile; onUpdate: (next: Partial<Profile>) => void }) {
  ...
  return <article className="panel voice-settings-panel" dir={profile.language === 'ur' ? 'rtl' : 'ltr'}>
    ...
    <small className="voice-note">No microphone is used. Urdu recordings can be added later; this version uses installed browser voices when available.</small>{!voice.urduVoiceAvailable && <small className="voice-warning">No Urdu system voice is installed on this device. Urdu will use the browser’s Urdu language request until an Urdu voice is added.</small>}
  </article>;
}
